...
*/,
  (L.HeatLayer = (L.Layer ? L.Layer : L.Class).extend({
    ...
    _initCanvas: function () {
      ...
      t.style[i] = "50% 50%";
      var a = this._map2.getSize();
      (t.width = a.x), (t.height = a.y);
      var s = this._map2.options.zoomAnimation && L.Browser.any3d;
    ...
    _reset: function () {
      var t = this._map2.containerPointToLayerPoint([0, 0]);
      L.DomUtil.setPosition(this._canvas, t);
      var i = this._map2.getSize();
    ...
    _redraw: function () {
      var t,
        ...
        d = [],
        _ = this._heat._r,
        l = this._map2.getSize(),
        m = new L.Bounds(L.point([-_, -_]), l.add([_, _])),
        c = void 0 === this.options.max ? 1 : this.options.max,
        u =
          void 0 === this.options.maxZoom
            ? this._map2.getMaxZoom()
            : this.options.maxZoom,
        f = 1 / Math.pow(2, Math.max(0, Math.min(u - this._map2.getZoom(), 12))),
        g = _ / 2,
        p = [],
        v = this._map2._getMapPanePos(),
        w = v.x % g,
        y = v.y % g;
      for (t = 0, i = this._latlngs.length; i > t; t++)
        if (
          ((a = this._map2.latLngToContainerPoint(this._latlngs[t])),
          m.contains(a))
        ) {
    ...
    _animateZoom: function (t) {
      var i = this._map2.getZoomScale(t.zoom),
        a = this._map2
          ._getCenterOffset(t.center)
          ._multiplyBy(-i)
          .subtract(this._map2._getMapPanePos());
